...
function startGame(){
  GameStatus = "start";
  document.getElementById("status").innerHTML = "Game is lodaing";
}
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press Play buttonn to Start and Play ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function manualControl(character){
  ...
  if(character.live){
    ...
    if(noseX < 300 &&! noseX > 300 &&!noseY < 140){ 
